 ...
 */
export async function getServiceLogs(projectId, location, serviceId, requestOptions) {
  if (!loggingClient) {
  ...
  try {
    ...
    console.log(`Fetching logs for Cloud Run service ${serviceId} in project ${projectId}, location ${location}...`);
    ...
    const [entries, nextRequestOptions, apiResponse] = await loggingClient.getEntries(options);
    ...
    const nextOptions = apiResponse?.nextPageToken ? nextRequestOptions : undefined;
